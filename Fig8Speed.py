...
currentY=0
TIME=12 #Seconds
pointsPerSecond=400/TIME
timePerPoint=1/pointsPerSecond

for i in range(1,200):
    nextX=currentX+(0.8*0.01*math.cos((180*((i-1)*0.01))*(math.pi/180)))
    currentX=nextX
    nextY=currentY+(-0.8*0.01*math.sin((180*((i-1)*0.01))*(math.pi/180)))
    currentY=nextY
    POSITIONARRAY.append([currentX,currentY])
for i in range(201,400):
    nextX=currentX+(0.8*0.01*math.cos((180*((i-201)*0.01))*(math.pi/180)))
    currentX=nextX
    nextY=currentY+(0.8*0.01*math.sin((180*((i-201)*0.01))*(math.pi/180)))
    currentY=nextY
    POSITIONARRAY.append([currentX,currentY])
...
OrigPath='//home/bitcraze/Documents/DRONE SPEED FLIGHT/UNTUNED PID/'
PATH = OrigPath + timeStamp + '.csv'
PATH2= OrigPath+'checklist.csv'
maskRadius=.5
			...
			#landing
                        ...
                        with open(PATH, mode='w') as file:
                                ...
                                for entry in logger:
                                        ...
                                        if point_x**2 + (point_y+.255)**2>maskRadius**2 and point_x**2 + (point_y-.255)**2>maskRadius**2:
                                                failed=True
                                                print("Out of Bounds")
                        ...
                        plt.show()
                        #plt.savefig(OrigPath+'Graphs/'+timeStamp+'.png')
